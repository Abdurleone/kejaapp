...
- Bottom navigation: only 2 tabs are pinned per role — Dashboard plus that role's one signature feature (Discover for tenants, Workspace for landlord/agency, Movers for movers, Admin for admins) — plus a third **More** tab. Tapping More opens a menu (`MoreScreen`, inside `MoreStack`) listing every other role-visible screen as a row (icon, label, chevron, with an unread-count badge on the Notifications row); tapping a row pushes that screen while More stays highlighted as the active tab, mirroring the classic iOS "More tab" pattern rather than cramming every role-visible screen onto the bar as its own button. Which two tabs are pinned per role lives in `navigation/roleTabs.js` (`getPrimaryTabs`/`getHiddenTabs`).
...
└── src/
    ...
    ├── navigation/           # Root stack, bottom tabs (roleTabs.js: pinned tabs + hidden/More split), Discover/Workspace/Movers/Admin/More stacks
...
`npm test` (Jest + React Native Testing Library, `jest-expo` preset; 32 suites, 186 tests). Coverage spans:
- Pure-function modules: `screens/workspace/propertyForm.js`, `navigation/roleTabs.js` (including the `getPrimaryTabs`/`getHiddenTabs` pinned-vs-More split), `utils/format.js`.
...
- Every pinned-tab and More-menu screen and its sub-screens (RNTL render tests, with `../api/index.js` and the relevant context hooks mocked): `DashboardScreen`/`LandingView`, `DiscoverScreen` and its sub-screens (`PropertyDetailScreen`, `InquiryFormScreen`, `ViewingRequestFormScreen`, `MoverRequestFormScreen`), `SavedScreen`, `WorkspaceScreen`/`PropertyEditScreen`, `MoversScreen` (directory and mover-dashboard views), `RequestsScreen`, `NotificationsScreen`, `FeedbackScreen` (submitter and admin-responder views), `AccountScreen`, `AdminScreen`/`AdminUserDetailScreen`, `MoreScreen` (the More-menu list), and the auth screens (`LoginScreen`, `RegisterScreen`).
