...
// bar would, so MainTabs.js's role-based tabs keep working unchanged. Styled
// with the same matatu-poster tokens (colors.stroke/shadow/radius,
// WorkSans bold labels) every other piece of mobile chrome already uses.
//
...
export default function LiquidTabBar({ state, descriptors, navigation }) {
  ...
  const activeTint = focusedOptions.tabBarActiveTintColor || colors.green;
  const inactiveTint = focusedOptions.tabBarInactiveTintColor || colors.muted;
  // A translucent surface tint under the blur, matching the tinted-glass
  // look every other surface's shadow/stroke pairing implies.
  const tintOverlayColor = `${colors.surface}b3`;
  ...
  return (
    ...
    >
      <View style={[styles.pill, { borderColor: colors.stroke, borderWidth: colors.strokeWidthSm }, colors.shadow]}>
        <BlurView
          intensity={70}
        ...
        {state.routes.map((route, index) => {
          ...
          return (
            ...
            >
              ...
              <Text style={[styles.label, boldText, { color: tintColor }]}>{label}</Text>
