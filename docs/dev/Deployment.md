...
Since **publish** has never actually run (blocked by the billing lock since it was written), `ghcr.io/abdurleone/kejaapp-backend:latest` was manually pre-seeded once — built from the exact same plain `backend/Dockerfile` this job uses, so there's no drift once CI genuinely runs it for the first time and takes over the tag. **`v1.0.0` is a separate, permanent tag on the same package name, built from `backend/Dockerfile.render` instead** (the actual Render production image, frontend bundled in) — a one-time version snapshot published alongside the [v1.0.0 GitHub Release](https://github.com/Abdurleone/kejaapp/releases/tag/v1.0.0), not something this CI job ever creates or will overwrite (it only ever pushes `latest` and a commit SHA). Don't confuse the two: `latest` tracks the k8s-oriented, API-only image; `v1.0.0` is the bundled Render image. The package is currently private and not yet linked to the repo - GitHub only auto-links a package pushed with a workflow's own `GITHUB_TOKEN`, or via a manual "Connect repository" step in the package's web settings (no REST API for it) - worth doing once **publish** runs for real.
