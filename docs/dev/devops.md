...
2. Set the one secret the blueprint deliberately leaves blank: `MONGODB_URI` on **jakezapp-backend** (Atlas connection string, including credentials). `JWT_SECRET` is auto-generated by Render; everything else has a value in `render.yaml`.
3. Create an object storage bucket for property images (see "Object storage" below) and set the five `sync: false` secrets on **jakezapp-backend**: `S3_BUCKET`, `S3_ENDPOINT`, `S3_ACCESS_KEY_ID`, `S3_SECRET_ACCESS_KEY`, `S3_PUBLIC_BASE_URL`.
4. Deploy. Once the service is up, confirm both the page and its API work from the same URL (check the browser console/network tab for CORS or CSP errors) — `CORS_ORIGIN`/`VITE_API_BASE_URL` in `render.yaml` assume the default service name `jakezapp-backend`. If Render assigns a different subdomain (name already taken, custom domain, etc.), update both to match and redeploy.
...
- `s3` — writes to any S3-compatible bucket via `@aws-sdk/client-s3`. This is what `render.yaml` sets for **jakezapp-backend**.
...
Setup: create a project at [sentry.io](https://sentry.io) (Node/Express platform), copy its DSN, set `SENTRY_DSN` on **jakezapp-backend**.
...
flowchart LR
    ...
    subgraph RenderTarget["Render (production)"]
        direction TB
        RenderEdge["jakezapp-backend web service<br/>(one URL for both)"]
