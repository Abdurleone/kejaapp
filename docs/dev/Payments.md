...
3. Generate a random `MPESA_CALLBACK_SECRET` (e.g. `openssl rand -hex 32`) and set `MPESA_CALLBACK_URL` to include it as the final path segment: `https://<host>/api/support-payments/callback/<the-secret>`. Unlike Safaricom's C2B API (which has a separate `registerurl` call), STK push needs no such step - `services/mpesaService.js` sends `CallBackURL` directly in every `stkpush/v1/processrequest` request body, so setting the env var and redeploying is the whole job. This secret is never exposed to any client, unlike `CheckoutRequestID`.
