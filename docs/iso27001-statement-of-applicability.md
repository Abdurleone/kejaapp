...
| 8.13 | Information backup | P | `npm run backup`/`npm run restore` (`backend/scripts/`) dump every collection to gzip-compressed EJSON and upload it to a separate, private S3-compatible bucket (deliberately not the public-read `S3_BUCKET` property-image bucket - see [`docs/devops.md`](devops.md#backups-and-disaster-recovery)). One real restore drill performed and verified. Gap remaining: it's a manual, human-triggered procedure, not an automated schedule - see [Section 7](#7-remediation-roadmap). |
