# JakezApp Code of Ethics

## 1. Purpose and scope

This Code of Ethics sets out the principles that govern how JakezApp is built, operated, and used. It applies to:

- The JakezApp team (engineering, product, support, and anyone with administrative access to the platform).
- Every user role the platform serves — tenants, landlords, agencies, movers, and admins.

It is a **platform and business ethics policy** — how JakezApp treats the people who use it and the decisions its operators make. It is distinct from [`CODE_OF_CONDUCT.md`](../CODE_OF_CONDUCT.md), which governs behavior of permitted contributors to the codebase itself (based on the Contributor Covenant).

The principles here are made enforceable in the [Terms of Service](terms-of-service.md) and the [Acceptable Use Policy](acceptable-use-policy.md); how JakezApp handles disagreements — between users, or about an enforcement decision — is covered in the [Dispute Resolution & Complaints Policy](dispute-resolution-policy.md).
...
- JakezApp does not manipulate search results, ratings, or reviews for commercial advantage. Ranking and filtering logic is applied uniformly to every listing that matches a search.

### 2.2 Non-discrimination and fair access

- JakezApp does not permit discrimination in housing access on the basis of ethnicity, tribe, race, religion, sex, marital or family status, disability, health status (including HIV status), or any other ground prohibited under Article 27 of the Constitution of Kenya.
...
- Every user has the right to know what data JakezApp holds about them, to correct it, and to request its deletion. See the Data Protection Policy for the full detail.

### 2.4 Safety and trust

- JakezApp actively guards against fraud and abuse: duplicate-listing-image detection, a violation-tracking system, and automatic suspension after repeated violations exist to protect tenants from fake or misleading listings.
...
- The automatic-ban threshold (4 active violations) is a fixed, known number, which carries a Goodhart's Law risk: a user aware of the exact count could try to stay just under it rather than change the underlying behavior. This is mitigated by the count being built from individually admin-adjudicated violations rather than an automated score, and by the [Dispute Resolution Policy](dispute-resolution-policy.md)'s review path for a wrongly-issued violation — the threshold is a backstop on human judgment, not a replacement for it. The same caution should apply to any future feature that surfaces a single number as a target (e.g. a landlord "trust score" or ranking metric) — see the note in [`qa-qc-report.md`](../project/qa-qc-report.md) on why test coverage is kept a diagnostic, not a gate, for the identical reason.

### 2.5 Fair dealing between roles

- JakezApp is a discovery and coordination platform. It does not mediate, hold, or process rent, deposits, or agency fees between parties (see [Payment Boundary](../../README.md#payment-boundary) in the README) — financial arrangements remain a direct matter between tenant and landlord/agency, and between tenant and mover. The one M-Pesa integration that exists (see [Payments](../dev/Payments.md)) is a voluntary service charge paid directly to the developer, not to any other user — it doesn't cross this boundary.
...
2. Accumulating active violations leads to account suspension, and a fourth active violation triggers an automatic ban, per the existing trust-and-safety design (see [Architecture — Trust & safety](https://github.com/Abdurleone/jakezapp/wiki/Architecture)).
...
- **Users**: use the in-app Feedback tab, or email `privacy@jakezapp.com` for anything involving personal data or account security.
