...
| CC8 | Change Management | P | A PR-based workflow is the practiced convention, but it isn't enforced: the `main` branch ruleset requires a PR to exist, not an approval or a passing CI run — and the `CI` GitHub Actions workflow, though enabled, fails every run immediately due to an account-level GitHub billing lock (re-confirmed through 2026-08-25; SoA 8.4, 8.25, 8.32). An auditor would very likely treat "no enforced review, no working automated test gate" as a material exception in this category, not a minor note. |
...
1. **CI blocked by an account billing lock, no enforced code review gate.** This is now the standout Change Management finding (CC8) — a workflow that exists in the repo, is enabled, but has never successfully completed a run since 2026-07-06 (an account-level lock, re-confirmed through 2026-08-25) reads worse to an auditor than one that was never built, since it demonstrates the control is currently non-functional rather than simply never prioritized.
