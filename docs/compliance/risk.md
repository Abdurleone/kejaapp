...
| 4 | [Expo-SDK-tied mobile dependencies can silently break tooling](#44-mobile-dependency-drift-a-demonstrated-repeat-failure-mode) — already happened 3 times | High (demonstrated 3x) | Medium | **High** |
...
| 10 | [Two open Dependabot alerts, both DoS-class, deliberately unforced](#410-known-deliberately-accepted-dependency-alerts) | Medium | Low | **Low** |
...
Not hypothetical — this exact failure mode has now happened **three times**, each caught only by a manual QA pass, never by CI (which can't run, see item 1):
...
3. **`react-native` itself**, a plain *minor* bump (0.86.2 → 0.87.0, PR #269), broke the entire mobile test suite outright (`jest` couldn't even start) — undetected through that PR and one more (#284) because both were verified against a stale `node_modules` rather than a clean install. See `docs/project/qa-qc-report.md`'s most recent snapshot and `CHANGELOG.md`'s "QA/QC Refresh" entry for the full incident writeup.

**Root cause common to all three**: these packages' real-world compatible version is dictated by the installed Expo SDK (`npx expo-doctor`/`npx expo install --check` is the actual source of truth), not by semver — and Dependabot's group-bump automation has no way to know that. `.github/dependabot.yml` now has explicit `ignore` rules for all three cases (the two majors, plus `react-native` itself blocked from any automated bump at all) — a real, incrementally-built guard rail, but one that only covers packages that have *already* caused an incident. Any other Expo-bundled native package (`react-native-worklets`, `react-native-screens`, `react-native-svg`, `react`/`react-dom`, etc.) could drift the same way and wouldn't be caught until it does.

**Rated High**: near-certain to recur in some form (three strikes and the pattern isn't fully closed), with a demonstrated impact ceiling of "the entire mobile test suite is unrunnable," not just a single broken feature.
...
Every mobile QA pass and every live-device verification recorded in `CHANGELOG.md`/`docs/project/live.md` has been on a real Android emulator or Expo Go — iOS has never been run on a real device or simulator. Given the volume of platform-specific native code involved (Reanimated/worklets, `expo-secure-store`'s Keychain backing, push notification entitlements), an iOS-only regression is plausible and would currently ship undetected.
...
Two open Dependabot security alerts as of this snapshot, both in `mobile/package-lock.json`, both DoS/ReDoS-class (not injection/RCE/auth-bypass): `image-size` (infinite-loop DoS in its ICNS/JXL/HEIF parsers) and `minimatch` (ReDoS). Both are transitive dependencies of `eas-cli`/`metro`'s own dependency chain — fixing either requires `npm audit fix --force`, which would downgrade `eas-cli` to a nonsensically old version or `expo` back to an unsupported SDK, a worse trade than the alerts themselves (see `docs/project/qa-qc-report.md` and `Roadmap.md`'s "Next" section for the full history — down from 26 originally, most resolved naturally via routine bumps). `eas-cli` is a locally-run dev tool never exposed to untrusted input in normal use, which further lowers the real-world exercise path for either.
...
3. **Expand the `dependabot.yml` ignore pattern** from "block the three packages that already broke something" to a documented policy of never auto-merging *any* Expo-SDK-bundled native package without an `expo-doctor` check first — the current list is reactive, not preventive.
