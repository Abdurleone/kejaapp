...
**Compliance pass, 2026-08-11 (commit `cc554b8`)**: a dedicated documentation-accuracy pass (not a fresh full re-verification of every control) found three real, undocumented changes since the 2026-07-23 update above — all shipped in the days immediately after it, none reflected here or in the docs this SoA cites as evidence: (1) the **Render consolidation** (PR #221) made production genuinely same-origin and decoupled `sameSite` from `secure` on the auth/CSRF cookies (`sameSite: "lax"` on Render now, still `"none"` on the cross-origin docker-compose/Kubernetes paths) — `5.14` and `5.15` updated to describe both shapes instead of only the old cross-origin one; a stale code comment in `backend/middlewares/csrfProtection.js` asserting the cookie is "necessarily `sameSite: none` in production" was also corrected. (2) The Render consolidation gave the bundled frontend its **first-ever Content-Security-Policy** (PR #223, fixing a real live bug it introduced — property images were blocked by Helmet's default `img-src`) — credited under `8.20`. (3) A **`.dockerignore` hardening** (PR #224, found during a post-merge security review of the same consolidation) closed a real but not-yet-exploited secret-file exposure gap opened when the Docker build context widened to the repo root — credited under `8.9`. Separately, `docs/devops.md`'s "Network topology" diagram (cited as evidence for `8.20`/`8.22`) had gone stale — it still depicted Render with a retired separate static-site host — and was corrected in the same pass to show Render and Kubernetes as the genuinely different shapes they now are.

**Compliance pass, 2026-08-25 (commit `bba1aad`)**: a documentation-accuracy pass found the CI narrative above had gone stale in a way that mattered - `8.4`/`8.25`/`8.29`/`8.32` and the Remediation Roadmap all described CI as `disabled_manually`, a deliberate team decision. That's no longer true: CI was re-enabled on GitHub Actions since the 2026-08-11 pass, but every run since fails immediately with an account-level GitHub billing lock (re-confirmed repeatedly - 2026-08-06 through 2026-08-25, most recently via `gh run list` on the merge commits above - zero job steps ever executing). The practical effect (no automated CI signal) is unchanged, but the cause is now an account-level lock outside this repo's control, not a decision anyone is deferring - corrected everywhere below rather than left as historical color. Also corrected: `8.29`'s stale speculation that a later Dependabot run succeeding meant the lock had cleared (it hadn't - definitively contradicted by every re-check since); `8.16`'s Sentry status (the backend's production DSN is now actually set and live, closing that half of the gap - only mobile's remains outstanding, an EAS secret not yet configured); and this document's own self-reference, which was missing its `compliance/` path segment.
...
| 5.14 | Information transfer | Y | HTTPS required in production; CORS origin allow-list restricts browser access on the cross-origin docker-compose/Kubernetes deployments. The consolidated Render production deployment (since PR #221) is same-origin, so CORS is less load-bearing there, but the allow-list still applies uniformly rather than being conditioned on deployment shape. A Content-Security-Policy (see `8.20`) further restricts what the bundled frontend HTML can load. |
| 5.15 | Access control | Y | JWT + RBAC (`protect`/`authorize` middleware) enforced on every protected endpoint; CSRF protection (`backend/middlewares/csrfProtection.js`, added since the previous review) requires an `Authorization` header on every state-changing request, closing the cookie-riding-along gap a session cookie otherwise leaves open — `sameSite: "none"` on the cross-origin docker-compose/Kubernetes deployments, `sameSite: "lax"` on the consolidated same-origin Render deployment (see `8.20`) — applied uniformly rather than conditioned on which shape is in play. Database-level access control (who/what can reach MongoDB at all, and how per-user data scoping is enforced) is documented separately in the [Database Access Policy](database-access-policy.md). |
...
| 5.30 | ICT readiness for business continuity | P | `backend/scripts/backupDatabase.js`/`restoreDatabase.js` (self-hosted - Atlas's own scheduled snapshots need an M10+ cluster, not the free M0 tier this project runs on) - a real restore drill was performed against a scratch MongoDB instance and confirmed byte-for-byte, including BSON type fidelity, not just document counts. Still manual/on-demand only, not scheduled - see [Section 7](#7-remediation-roadmap) and [`docs/devops.md`'s "Backups and disaster recovery"](../dev/devops.md#backups-and-disaster-recovery). |
...
| 8.3 | Information access restriction | Y | Ownership + role checks scope every list/detail endpoint (e.g. `fetchMyProperties`, `/inquiries/received`) — no client ever holds a database credential, so this application-layer scoping is the entire access boundary. See the [Database Access Policy](database-access-policy.md) for the full model, data classification, and known gaps (unverified Atlas user privilege, backups still manual not automated). |
...
| 8.9 | Configuration management | Y | Centralized, validated environment config (`backend/config/env.js`); no hardcoded secrets in source. The root-level `.dockerignore` was broadened to a general `**/.env*` pattern (previously three specific suffixes) after the Render consolidation widened the Docker build context to the repo root — closes a secret-file exposure gap that no `COPY` step actually exploited yet, but existed. |
...
| 8.13 | Information backup | P | `npm run backup`/`npm run restore` (`backend/scripts/`) dump every collection to gzip-compressed EJSON and upload it to a separate, private S3-compatible bucket (deliberately not the public-read `S3_BUCKET` property-image bucket - see [`docs/devops.md`](../dev/devops.md#backups-and-disaster-recovery)). One real restore drill performed and verified. Gap remaining: it's a manual, human-triggered procedure, not an automated schedule - see [Section 7](#7-remediation-roadmap). |
...
| 8.20 | Networks security | Y | CORS allow-list (fails closed on an unrecognized origin) and Helmet security headers; tiered rate limiting on `/api` (stricter on `/api/auth`, `backend/middlewares/rateLimiter.js`, added since the previous review) mitigates brute-force/basic-DoS exposure. The consolidated Render deployment's bundled frontend also gets its first real Content-Security-Policy (`backend/app.js`, conditional on `hasBundledFrontend`) — Helmet's defaults plus an allowance for Google Identity Services and an `img-src` widened to `https:` for arbitrary-host property photos. `docs/devops.md`'s "Network topology" diagram documents the request path across both deployment targets — corrected in a compliance pass after drifting stale following the Render consolidation (it still showed Render with a retired separate static-site host). |
...
| 8.22 | Segregation of networks | Y | Still delegated to the Kubernetes/Render network model rather than an explicit internal segmentation control, but `docs/devops.md`'s "Network topology" diagram documents it — corrected in a compliance pass to reflect that Render and Kubernetes are no longer the same shape (Render is now single-origin since the consolidation, Kubernetes stays two-origin on purpose) — not cross-checked against a live cluster's actual `kubectl get all` output. |
...
| 8.25 | Secure development life cycle | P | The `CI` GitHub Actions workflow (`.github/workflows/ci.yml`, lint + tests for all three packages) was re-enabled after this SoA's earlier passes recorded it as `disabled_manually` - but every run since fails immediately with "your account is locked due to a billing issue" (an account-level GitHub billing lock, re-confirmed repeatedly through 2026-08-25, zero job steps ever executed). The net effect is the same as being disabled - no automated CI signal exists either way - but the cause is now an account-level lock outside this repo's control, not a deliberate team decision to keep it off. Verification is currently manual: `npm test`/`npm run lint` run locally before each PR, per the CHANGELOG's established convention — a real discipline, but not an automated gate. |
...
| 8.29 | Security testing in development and acceptance | P | 585 backend unit/integration tests cover auth/authorization guard-rails, run manually (see `8.25`). A `CodeQL` (SAST) workflow is configured but has only run once, on 2026-07-06, and that run failed — root cause identified via `gh run view`: "account is locked due to a billing issue", not a code or config problem. The lock has **not** cleared since - re-confirmed repeatedly through 2026-08-25 via multiple `gh run list`/`gh run view` checks, all failing identically with zero job steps executed - so CodeQL's default-setup workflow remains unable to run at all. No penetration test or DAST tool run yet either. |
...
| 8.25 | Secure development life cycle | P | The `CI` workflow is enabled but every run fails immediately with an account-level GitHub billing lock (re-confirmed through 2026-08-25); verification is currently manual per-PR only. |
...
| # | Control | Status | Gap |
|---|---|---|---|
| 5.30 | ICT readiness for business continuity | P | A real disaster-recovery restore drill has now been performed (self-hosted scripts, not Atlas's own snapshots - free M0 tier doesn't support those) - the remaining gap is that it's manual, not scheduled. |
| 8.13 | Information backup | P | Self-hosted backup/restore scripts exist and one real restore was verified byte-for-byte. The remaining gap is automation - no scheduler is currently wired up (see `docs/devops.md` for the three options considered and deferred). |
...
| 8.16 | Monitoring activities | P | Sentry now covers application errors (backend + mobile in local dev); the backend's production DSN is set and live. Uptime alerting is now live (two UptimeRobot HTTP monitors on `/api/health/live`/`/api/health/ready`, email alert) — the remaining gaps are a dashboarding layer beyond UptimeRobot's own basic status page, and getting mobile's production DSN set (an EAS secret, still outstanding). |
...
1. **Getting the account billing lock cleared is the actual prerequisite now**, not re-enabling CI - that part is already done. The `CI` workflow was re-enabled on GitHub Actions since the passes above recorded it as `disabled_manually`, but every run since fails immediately with "your account is locked due to a billing issue" - re-confirmed repeatedly through 2026-08-25 with zero job steps ever executing. This is an account-level lock outside this repo's control, not a decision the team is deferring. A required status check against a workflow that can never complete would block every future merge indefinitely (this ruleset's `current_user_can_bypass` is `never`), so this cannot be revisited independently of the billing lock being cleared.
...
1. ~~Automated database backup + one tested restore~~ — **partially done**: Atlas's built-in scheduled snapshots turned out to need an M10+ cluster (not available on the free M0 tier this project runs on), so a self-hosted alternative was built instead - `backend/scripts/backupDatabase.js`/`restoreDatabase.js`, uploading to a separate private bucket. One real restore drill was performed against a scratch MongoDB instance and verified byte-for-byte. What's still open: it's manual/on-demand, not on an automated schedule - real automation (Render Cron, a GitHub Actions `schedule:` workflow, or a Kubernetes `CronJob`) was considered and deliberately deferred; see `docs/devops.md`'s "Backups and disaster recovery" for why. Closes the "no drill has ever been performed" half of `5.30`/`8.13`, not the "automated" half.
...
1. **Independent security testing** (`8.29`, `5.35`) — clarified, not resolved: `CodeQL`'s one failed run was traced to an account billing lock (`gh run view`'s annotation: "account is locked due to a billing issue"), not a code problem. The lock has **not** cleared - re-confirmed repeatedly through 2026-08-25, all runs failing identically - so CodeQL still cannot run at all. A real penetration test or DAST tool run is still entirely outstanding regardless, and now has no automated-scanning fallback either while the lock persists.
...
This SoA should be revisited whenever a new control-relevant capability ships (a new auth mechanism, a new third-party processor, a new deployment target) and at minimum reviewed annually. Maintained in `docs/compliance/iso27001-statement-of-applicability.md`.
