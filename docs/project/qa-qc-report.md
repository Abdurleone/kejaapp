...
- **Coverage tooling:** `npm run test:coverage` runs `node --experimental-test-coverage --test` — Node's built-in coverage reporter, zero new dependencies. Opt-in, a diagnostic report not a gate, for the same reasons as the previous snapshot: a coverage percentage that becomes a target invites tests written to move the number rather than test real behavior (Goodhart's Law) — see the same caution generalized in the [Code of Ethics](../compliance/code-of-ethics.md#24-safety-and-trust).
