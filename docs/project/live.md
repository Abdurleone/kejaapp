...
| Demo environment (`kejaapp-demo`, a separate Render Blueprint from `render.demo.yaml`) | **Provisioned in code, not yet deployed - URL not yet assigned, update this row once it is.** Its own Atlas database, no Redis/S3/Sentry/VAPID/M-Pesa (all "empty = disabled" by design). Reset via `npm run reset-demo-data` or `POST /api/demo/reset/<secret>`, not yet scheduled - see `Roadmap.md`'s Next section |
...
- **Support KejaApp's callback-delivery issue** (see Infrastructure above) - Daraja accepts the sandbox payment request but Safaricom's async callback never arrives. Real production Daraja credentials (an actual paybill/till) still need setting up once that's resolved. See `docs/dev/Payments.md`.
- **Scheduled notification jobs have no trigger on Render** (see Infrastructure above) - viewing reminders/review prompts/nudges aren't firing for real users.
