...
**A real, currently-live bug is merged but NOT yet deployed** - see "What's pending" below for the full status. Pick this up first tomorrow.
...
- **Login fix (PR #276) merged to `main` but blocked from deploying by a Render/Google Cloud outage - resume here first**: `loginUser` (`authController.js`) was fixed so a Google-only account (real `googleId`, no password) attempting a password sign-in gets a clean "Invalid credentials" instead of a raw `Path \`password\` is required.` Mongoose error - see `CHANGELOG.md`'s "Fix: Google-Only Accounts..." entry for the full root cause. **Confirmed still NOT live** as of 2026-08-20 ~17:00 UTC: a direct test against the real production `/api/auth/login` endpoint (a throwaway test account, created and cleaned up in the same live database, no real user affected) still returned the old raw error. Every Manual Deploy attempt failed with a "Service Unavailable" dialog and zero build logs - traced to a genuine Render status-page incident ("Deployment Issues," identified 2026-08-20 16:12 UTC, upstream Google Cloud problem, confirmed via `status.render.com`'s own API): free-tier Web Service builds/deploys/spin-up were deliberately disabled platform-wide while Render mitigates it. The currently-running container is healthy and serving traffic throughout - just stuck on the pre-fix commit (`4485751`, PR #273) until Render lifts the freeze. **Next step**: check `https://status.render.com` (or ask to have it checked) for this incident's resolution, then trigger Manual Deploy → Deploy latest commit, then re-run the same safe verification to confirm the fix actually landed.
