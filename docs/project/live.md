...
| Error tracking (Sentry) | Backend: **live in production** - `SENTRY_DSN` set on `kejaapp-backend` in Render (no independent public check possible, unlike VAPID's key endpoint - recorded on the account owner's confirmation). Mobile (`mobile/App.js`, `EXPO_PUBLIC_SENTRY_DSN`): **verified working locally** (real test event confirmed landing in Sentry, `200`), but no EAS secret set yet, so mobile crash reports still aren't reaching Sentry |
