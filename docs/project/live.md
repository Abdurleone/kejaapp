...
- `https://kejaapp-backend-7iu3.onrender.com` — one URL for both the web app and its API (still named `kejaapp-backend` after the JakezApp rebrand: Render doesn't change a service's `.onrender.com` URL on a rename, and getting a `jakezapp-backend` URL would mean creating a brand-new service and migrating traffic/data over for a purely cosmetic win - kept stable on purpose, see CHANGELOG.md's rebrand entry). There used to be a separate `kejaapp-frontend.onrender.com` static site; it's retired, not renamed — see CHANGELOG.md's "Consolidate Web + API onto One Render Origin" entry for why (closing out a cross-origin CSRF cookie problem at the source).
