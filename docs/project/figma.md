...
- **The inquiry/viewing-request modals on the web Property Detail page aren't captured.** While driving the "Send inquiry" and "Request viewing" buttons via headless-browser automation, clicks (mouse and keyboard) reached the correct, focusable button element but consistently didn't trigger the expected modal — reproduced with the same non-result across roughly a dozen attempts and several distinct click methods, with real UI interactions on the *same* page (nav tabs, the Save button, sign-in) working reliably throughout. This may be an automation-environment-specific interaction quirk rather than a real product bug — it's flagged here as worth a quick manual click-through in an actual browser to confirm one way or the other, not asserted as a confirmed defect.
