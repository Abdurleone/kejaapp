...
| Data minimization design | The coordinate is used to compute a distance **at read time** and is never persisted independently — see `attachDistanceKm` in `backend/controllers/moverRequestController.js`. If the underlying property's location changes, the distance recalculates rather than going stale, and there is no historical location trail to secure or later delete. |
