...
- Mobile lint is currently broken outright, not just held back: `eslint@10.6.0` crashes on the first file (`eslint-config-expo`'s vendored `eslint-plugin-react` still calls a `context.getFilename()` API that ESLint 9+'s flat config removed), so `npm run lint:mobile` isn't actually enforcing anything today. `jest-expo`'s `@react-native/jest-preset` (tied to RN 0.86) separately pins `babel-jest`/`jest-environment-node` to `^29.7.0`, incompatible with `jest@30` — revisit both once compatible releases publish.
- Most web pages (`AdminPage`, `WorkspacePage`, `MoversPage`, `FeedbackPage`, `AccountPage`, `PropertyEditPage`/`PropertyCreatePage`) are still only covered by `page-components.test.js`'s regex-source-matching, not real Vitest render tests — only `DiscoverPage`, `PropertyDetailPage`, `AuthModal`, `SavedPage`, and `DashboardPage` have genuine render+interaction coverage so far.
- Accessibility gaps found in a recent appraisal, not yet fixed: `AuthModal` has no dialog semantics (`role="dialog"`/`aria-modal`), focus trap, or Escape-to-close; the app's tab bar declares `role="tablist"` but its buttons don't implement the rest of the ARIA tablist pattern (`role="tab"`, `aria-selected`, arrow-key navigation).
