...
- Most web pages (`AdminPage`, `PropertyEditPage`/`PropertyCreatePage`) are still only covered by `page-components.test.js`'s regex-source-matching, not real Vitest render tests — only `DiscoverPage`, `PropertyDetailPage`, `AuthModal`, `SavedPage`, `DashboardPage`, `WorkspacePage`, `MoversPage`, `FeedbackPage`, and `AccountPage` have genuine render+interaction coverage so far.
