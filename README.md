...
- Movers are full accounts (a `mover` role, self-registered like tenant/landlord/agency), going through the same admin verification workflow as agencies. Landlords/agencies can mark specific movers as trusted affiliates; on a property page, tenants see the owner's affiliated movers plus general movers within a configurable radius of that property, and can send a service request directly to a mover, who sees it as a received request they can accept, decline, or complete. Requests include a required home-size category (Studio/1BR/2BR/3BR/4BR+); when sent from a property page (drop-off already known), the tenant sees a price estimate weighing pickup-to-dropoff distance against home size — whichever is smaller (below a 10km threshold) plays a reduced role while the other dominates, on top of the mover's base price.
- Notifications are actionable, not just a read-only inbox: opening a `mover_request` notification (as the addressed mover) or a `viewing`/`inquiry` notification (as the addressed property owner) jumps straight to the Movers or Workspace tab with the specific request highlighted, ready to accept/decline or approve/reject, and marks the notification read as a side effect.
- Property owners can approve or reject viewing requests directly from the Workspace tab (a "Viewing requests" panel/segment on both web and mobile), backed by a cross-property `GET /api/viewings/received` inbox — previously only possible via a raw API call despite the backend having supported it from the start.
...
- `createMoverRequest({ mover, property, homeSize, message, preferredDate })` → `POST /api/mover-requests` (tenant only) — response includes a `distanceKm`/`priceEstimate` pair, computed on read and never persisted, so it can't go stale
...
- `fetchReceivedViewingRequests(query)` → `GET /api/viewings/received` (landlord/agency, across all their properties)
- `updateViewingRequestStatus(viewingRequestId, { status, reason })` → `PUT /api/viewings/:id/status`
...
- Property detail page (`/property/:id`) reached via "Details" from Discover or Saved — full description, cost summary, contact info, amenities, a "Movers for this move" section (the owner's affiliated movers plus verified movers nearby, each with an inline "Request service" form requiring a home-size selection and showing the returned price estimate once sent, since the drop-off is already this property's own location), and inline forms to send an inquiry or request a viewing (matching the mobile app's flow). Also shows the listing owner's name and, for agencies, a "Verified agency" badge once approved.
...
- Movers tab (`/movers`) — for tenants/landlords/agencies/anonymous visitors, a filterable directory of mover businesses (service type, county, rating) with a "Request service" action per card (tenants, also requiring a home-size selection; no property tied to this entry point, so no price estimate here) and an "Add/remove affiliate" action (landlords/agencies); for signed-in movers, their own profile-status panel (edit business details, see verification status) plus a list of received tenant requests with accept/decline/complete actions, and a highlighted/scrolled-to request when reached by tapping a `mover_request` notification.
...
- Saved properties list loading from real `/api/favorites` endpoint (paginated, with Previous/Next controls once there are more than one page), with a Details link into the same property detail page as Discover.
- Owner workspace showing the signed-in landlord/agency's own listings, the real inquiries tenants have sent about them, and a "Viewing requests" panel to approve/reject incoming viewing requests (all three scoped server-side by `owner`, not filtered client-side), with an Edit action on each listing card opening a full edit form (`/owner/properties/:id/edit`) backed by `PUT /api/properties/:id`, plus a "New listing" action (`/owner/properties/new`) backed by `POST /api/properties` for creating new properties. Tapping a `viewing`/`inquiry` notification jumps here with the relevant card highlighted. Tenants only ever get read access to listings (Discover/Saved/property detail) — creation and editing are gated behind `canManageListings` (landlord, agency). Admins cannot create, edit, or view the owner workspace at all — admins moderate accounts, not listings.
- Admin console (`/admin`) for managing user accounts: search and filter users by role, open a user's account summary (violations, and role-specific activity counts) and status change history, and change an account's status to active, suspended, or banned with a reason, backed by the existing `GET/PUT /api/admin/users*` endpoints. A full-cascade `DELETE /api/admin/users/:id` also exists (an admin cannot delete their own account this way) for administrative cleanup — not yet exposed as a console button, currently reached only via the API or `backend/scripts/cleanupTestData.js`.
...
- Notifications tab (`/notifications`), visible to every signed-in role: lists all notifications with an "Unread only" filter and a mark-as-read action per item that persists across reloads. Actionable notifications (a `mover_request` for the addressed mover, a `viewing`/`inquiry` for the addressed property owner) also show a "View request" action that jumps to the Movers or Workspace tab with the specific card highlighted and marks the notification read as a side effect — scoped to just the actionable recipient, since the other side (e.g. a tenant told their viewing was approved) has no "my sent requests" tracking view to land on yet.
...
- Given I am a landlord or agency, when I open the "Viewing requests" panel/segment on the Workspace tab, then I see incoming viewing requests across all of my properties in one place (not one property at a time) and can approve or reject each directly, without needing a raw API call.
...
- Given I am a tenant, when I send a service request to a mover (optionally from a specific property page), then the mover receives it and I can see it pending. I must choose a home-size category (Studio/1BR/2BR/3BR/4BR+); if the request is tied to a property (so the drop-off location is already known), I also see a price estimate weighing pickup-to-dropoff distance against my chosen home size — whichever is smaller, below a fixed distance threshold, plays a reduced role while the other dominates, on top of the mover's base price.
...
- A live UI/UX appraisal covered the web frontend only (see `docs/qa-qc-report.md`'s cross-cutting section); the native mobile app still needs the same pass, but the dev Android emulator has now OOM-crashed three times attempting a cold boot — the host currently has 0 swap available (disabled while troubleshooting an earlier crash, not restorable from inside the container). Needs `wsl --shutdown` from a Windows-side terminal before that can be retried.
