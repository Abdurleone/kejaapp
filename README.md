...
- Most web pages (`AdminPage`, `WorkspacePage`, `MoversPage`, `FeedbackPage`, `AccountPage`, `SavedPage`, `DashboardPage`, `PropertyEditPage`/`PropertyCreatePage`) are still only covered by `page-components.test.js`'s regex-source-matching, not real Vitest render tests — only `DiscoverPage`, `PropertyDetailPage`, and `AuthModal` have genuine render+interaction coverage so far.
