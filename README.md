...
- Mobile: creating a listing now supports adding photos in the same flow, but editing an existing listing (and managing its photos afterward) is still web-only — there's no mobile edit screen at all yet. The received-inquiries view (replying to inquiries on your own listings) is also still web-only. The web admin console now covers user management, but mobile still has no admin console screen at all, and review moderation UI is still missing on both web and mobile.
