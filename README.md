...
- Most web pages (`AdminPage`, `FeedbackPage`, `AccountPage`, `PropertyEditPage`/`PropertyCreatePage`) are still only covered by `page-components.test.js`'s regex-source-matching, not real Vitest render tests — only `DiscoverPage`, `PropertyDetailPage`, `AuthModal`, `SavedPage`, `DashboardPage`, `WorkspacePage`, and `MoversPage` have genuine render+interaction coverage so far.
