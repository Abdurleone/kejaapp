...
`backend/jobs/` (viewing reminders, review prompts, stale-listing/inquiry nudges, push-receipt polling) are standalone scripts run via `npm run jobs`, deliberately not an in-process timer — but nothing currently invokes them on a schedule against production. The `k8s/backend-cronjob.yaml` `CronJob` that would do this only exists on the k8s path, which isn't live; Render has no Cron Job service configured. Don't assume these jobs are firing in production — see `docs/project/Roadmap.md`'s Next section for the fix options under consideration.
